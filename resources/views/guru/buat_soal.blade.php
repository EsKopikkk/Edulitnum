            ...
            <div class="bg-white rounded-lg shadow p-6" style="border: 1px solid #FFC81E;">
                <form method="POST" action="{{ route('soal.store') }}">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="pertanyaan" rows="3" class="w-full border rounded p-2 focus:ring-orange-500" style="border-color: #FFC81E;" required></textarea>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="flex items-center mb-2 gap-2">
                            <input type="radio" name="kunci_jawaban" value="{{ $p }}" required>
                            <span class="font-bold w-6">{{ $p }}.</span>
                            <input type="text" name="pilihan_{{ strtolower($p) }}" class="flex-1 border rounded p-2" style="border-color: #FFC81E;" placeholder="Isi pilihan {{ $p }}" required>
                        </div>
                        @endforeach
                        <p class="text-sm text-gray-500 mt-1">* Pilih bulatan radio untuk menandai kunci jawaban yang benar.</p>
                    </div>

                    <button type="submit" class="px-6 py-2 rounded text-white font-semibold" style="background-color: #E87F24;">Simpan Soal</button>
                    <a href="{{ route('soal.index') }}" class="ml-2 px-6 py-2 rounded text-white font-semibold bg-gray-400">Batal</a>
                </form>
            </div>
        </div>
    </div>
