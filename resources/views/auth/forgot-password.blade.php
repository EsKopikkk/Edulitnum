...
<head>
    ...
    <title>Lupa Password | Edulitnum</title>
    ...
    <div class="relative z-10 w-full max-w-md">
        <div class="glass-card rounded-[40px] p-10 md:p-12">
            ...
            <div class="text-center mb-10 relative">
                ...
                <h2 class="text-3xl font-black text-edu-dark tracking-tighter mt-6">Lupa <span class="text-edu-orange">Password?</span></h2>
                <p class="text-gray-500 text-sm mt-2 font-medium">Masukkan email terdaftar untuk reset password</p>
            </div>

            @if (session('status'))
                <div class="mb-6 px-4 py-3 bg-green-100 text-green-700 rounded-2xl text-sm font-semibold">
                    {{ session('status') }}
                </div>
            @endif
            ...
            <form action="{{ route('password.email') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="email" name="email" placeholder="Masukkan email terdaftar" class="input-field w-full px-6 py-4 rounded-2xl text-edu-dark font-semibold" required>
                    @error('email')
